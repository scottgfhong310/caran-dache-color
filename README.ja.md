...
> 版本 v1.1｜最終更新 2026-07-22

[繁體中文](README.zh-Hant.md) ｜ [English](README.md) ｜ **日本語**

**Caran d’Ache の色番号を CSS に対応させる**シングルページ参照 WebApp。**9 つの主要アーティストシリーズ**を
...
- **9 シリーズ** — LUMINANCE 6901、PABLO、SUPRACOLOR、MUSEUM Aquarelle、NEOCOLOR II、NEOPASTEL、
  PASTEL PENCILS、PASTEL CUBES、**NEOART 6901**（Pastel Pencils/Cubes は同一の公式 84 色パレットを共有；
  Pastel Cubes はレガシーパレット；NEOART 6901 — 48 色 — はマスターインデックス v1.1.0 で追加）。
- **812 シリーズ色**（色×シリーズごとに 1 行）· **227 正準色番号**（シリーズ横断で重複排除）。

## 特長

- **2 つの閲覧軸** — *シリーズ*（9 チップ → そのシリーズのグリッド）と *正準色番号*（227 番号、各々に
...
- **CSS エクスポート** — `caran_dache_colors.css` の表示 / コピー / ダウンロード（812 個の
  `--cda-<series>-<code>` 変数 ＋ `.cda-color-…` / `.cda-bg-…` ユーティリティクラス）。
- **最近色マッチャー** — サイドツールからピッカー/hex 入力で CIEDE2000（ΔE00）による最近 10 色を品質バンド付きで一覧、
  クリックで詳細へ（lib の `nearestCDA`；faber-castell-color の `nearestFC` 同様に他アプリへ埋め込み可）。
...
├─ data/cda-series.js                   # window.CDA_SERIES — 9 シリーズ
├─ data/cda-colors.js                   # window.CDA_COLORS — 812 シリーズ色（＋ window.CDA_META）
...
| `deltaE(labA, labB)` / `deltaEBand(dE)` | CIEDE2000（ΔE00）＋品質バンド（`very`/`close`/`noticeable`/`far`）|
| `nearestCDA({r,g,b}, {n,series,colors})` | ΔE00 で最も近い Caran d’Ache シリーズ色（PSTC はデフォルト重複除外；シリーズ絞り込み可）|
...
データは `Caran_dAche_Master_Color_Index_v1.1.0.xlsx` に由来し、これ自体は公式 Caran d’Ache カラーチャート
