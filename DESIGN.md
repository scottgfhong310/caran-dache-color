...
- **`nearestCDA`（v2，已實作）**——本 lib 的第二個身分，比照 FC 的 `nearestFC`（被 `color-palette`／
  `thangka-trace` 複製共用的「螢幕色 → 買得到的筆」橋）：給任一螢幕色，以 **CIEDE2000（ΔE00）** 找最接近的
  Caran d’Ache **系列色**（實體筆，非正典平均色）。設計取捨：
  - **參考庫＝812 系列色、預設排除 PSTC**——PSTC 與 PSTP 共用同一份官方調色盤、hex 逐碼相同，不排除會讓
  ...
  - 消費端（`color-palette`／`thangka-trace`）之後要用時，比照 FC 慣例**複製** lib＋`data/cda-colors.js` 過去即可。
